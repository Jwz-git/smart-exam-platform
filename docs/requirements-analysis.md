...
| 错题本 | 学生查看已公布成绩考试里未得满分的题目，客观题可重练 | 成绩公布前错题本为空；重练不改动答卷分数与名次 |
| 系统设置 | 管理员修改白名单内的运行参数 | 改及格线后成绩页与统计页的及格率立即重算，无需重启；密钥不可编辑 |

### 3.2 延后或排除

`plan.md` 第 2.3 节的六项增强功能已全部确认并实现：AI 辅助出题（服务商可自定义，兼容 OpenAI 与 Anthropic 两种协议，只生成可编辑草稿，教师确认后才走普通的题目新增接口保存）、规则自动组卷（按题型/难度/知识点抽题生成方案，保存仍走原组卷接口）、错题本与错题重练（只收录已公布成绩的未得满分题目，重练不改动成绩）、统计图表、题库批量导入与试卷导出（导入支持 CSV/TSV 且先预览后写库，导出的 CSV 可原样回导）、答题自动保存与异常恢复。系统设置另由只读改为管理员可编辑（白名单六项，密钥与连接串仍不可编辑）。在线监考、分布式部署、AI 主观题评分、支付和第三方教务集成不实现。
