...
| 组号与成员 | 第 1 组：24050417 郏尉植、24050415 陆佳明 |
| 报告日期 | 2026-09-04 |
| 代码仓库 | 本仓库，默认分支 `main` |
...
本报告按课程要求的五节组织：项目背景、需求分析、设计文档、系统演示、心得体会，并补充「运行结果分析」与「创新性」两节，对应评价要点中的「对程序运行结果的分析」和「项目实施思考」。报告中的每一个数字都能在仓库里复核，复核命令见附录。
...
本组 2 人，按「实现」与「设计文档」两条线分工，接口契约与验收数据由两人共同确认后固定下来：

| 成员 | 承担工作 | 主要交付物 |
|---|---|---|
| 24050417 郏尉植 | 前后端具体开发、API 接口设计、界面原型设计、数据库测试数据、项目测试案例设计、答辩 PPT 制作 | `backend/`、`frontend/`、[`api.md`](api.md)、[`界面原型设计.png`](images/界面原型设计.png)、[`test-records.md`](test-records.md)、答辩 PPT |
| 24050415 陆佳明 | 需求分析、系统设计、数据分析、业务流程图与程序流程图、E-R 图与数据库表设计、报告主体撰写 | [`requirements-analysis.md`](requirements-analysis.md)、[`technical-design.md`](technical-design.md)、[`database-design.md`](database-design.md)、本报告 |

**质量如何保证。** 课程评价要点里的「任务分工合理性」在本项目里对应一个具体问题：两人分头做的东西凭什么保证是一致的、正确的。本项目用四层可复核证据回答：
...
3. **浏览器完整走查并留存 39 张截图**，界面行为与接口响应互相印证，设计文档里写的交互与实际页面逐条对上。
...
- **范围边界**：面向课程演示规模（单实例部署、演示数据量），在线监考、大规模并发考试与分布式部署明确不在本期范围内，与 [`plan.md` 第 2.4 节](../plan.md#24-暂不实现)一致。
...
| 5 | 超时自动交卷判的是什么 | 答案在切题时和每 30 秒各同步一次到服务端 | 只在点「交卷」时才提交答案的话，服务端兜底拿到的是一张空白卷，超时未交的学生会被判 0 分——兜底机制本身也需要数据支撑 |
...
| 15 | 时间语义由谁决定 | 连接串强制 `connectionTimeZone=UTC&forceConnectionTimeZoneToSession=true`，答卷开始时间由后端显式写入 | 交给数据库 `CURRENT_TIMESTAMP` 默认值的时间会按会话本地时区写入、按 UTC 读出，产生固定 8 小时偏移；跨系统的时间语义必须是一个明确的设计决定 |
...
| 19 | 界面为什么要按角色请求接口 | 首页按角色分别取数：教师取题库与考试，管理员取账号构成 | 不分角色一律请求教师接口的话，管理员一登录就会收到四个 403，并看到一条对他毫无意义的报错——权限模型必须在取数这一层就体现出来 |
...
四项新增功能也全部在真实 MySQL 9.6 上走查（完整记录见 [`docs/test-records.md`](test-records.md) 第 6 节）：V5 迁移实测应用成功（`now at version v5`），新增 `system_setting` 与 `practice_attempt` 两张表且既有数据不受影响；自动组卷的方案原样保存成 30 分 4 题的试卷；导出的 CSV 被导入接口读回 4 行全部成功；管理员改及格线后成绩页及格率 25.0% → 75.0%、统计页及格线 12.0 → 10.0，恢复默认后回到原值；错题重练两次后错题本显示「练习次数 2、已掌握」而答卷总分仍是 28.0。浏览器走查另留存 9 张截图（`31`—`39`），界面行为与接口结果逐条对上。

批量导入同样在真实 MySQL 上逐条核对（完整记录见 [`docs/test-records.md`](test-records.md) 第 6 节）：模板五行示例导入 5 成功 0 失败，题库总数 11 → 16；预览前后题库总数不变，证明预览确实不写库；同一份内容再导一次得到 `imported=0 skipped=5`；题型写错、答案指向不存在的选项、分值写成 `abc`、判断题答案写「也许」四行各自失败并给出原文行号，同一批里的好行照常写入；缺必填列、只有表头、201 行三种整表错误分别返回 `IMPORT_INVALID_FORMAT` 与 `IMPORT_TOO_MANY_ROWS`；未登录 401、学生与管理员 403。
...
**第一条经验：跨系统的时间语义必须是一个明确的设计决定。** 答卷的开始时间如果交给数据库的 `CURRENT_TIMESTAMP` 默认值，它会按会话本地时区写入、按 UTC 读出，于是倒计时上会出现固定 8 小时的偏移。本项目的做法是连接串强制 UTC 会话时区，并让后端显式写入这个时间。「顺手交给数据库默认值」看起来省事，实际是把一个跨系统的语义约定丢给了两边各自的默认配置——而默认配置随时会因为换一台机器而改变。

**第二条经验：兜底机制本身也需要数据支撑。** 服务端的超时自动交卷如果只把状态从「作答中」改成「已交卷」，那么后端手上拿到的是一张空白卷，兜底逻辑等于把学生的成绩清零。因此答案在切题时和每 30 秒各同步一次到服务端，兜底判的才是真实作答。这类问题在功能测试里不会暴露，因为测试里的学生都会乖乖点「交卷」——只能靠事前想清楚「兜底那一刻，数据从哪里来」。

**第三条经验：共享的测试基础设施必须是幂等的。** 引入 AI 相关测试时用了 `@MockitoBean`，Spring 会为它另建一个应用上下文并重跑一次建表脚本，因此建表脚本必须可重复执行。这件事的价值不在修法本身，而在于它说明：表面症状（认证失败）和真实原因（测试基础设施）可以离得很远，定位成本远高于修复成本，所以基础设施的假设越少越好。

**一个反复出现的判断：安全边界只能放在服务端。** 成绩公布前的字段裁剪、题目归属校验、评分上界、公布前置条件，全部在后端实现，前端的按钮禁用只作为体验优化。做界面时很容易图省事只在前端拦——按钮灰了用户就点不到了。但浏览器里能看到的就等于已经泄露，`curl` 一下就能绕过所有前端逻辑。这也是最后专门用命令行验证越权路径的原因：界面上点不出越权，不代表接口拦得住。

**一个关于 AI 的判断：让 AI 只生成草稿。** 实现 AI 辅助出题时，最省事的做法是让模型直接写库。但那样模型的输出就绕过了全部业务校验——它可能给出四个选项却把标准答案写成 `E`，也可能把判断题的答案写成一句话。最后的做法是：题型、难度、知识点、分值由人指定，模型只创作内容；输出必须过和手工出题完全相同的校验；保存走同一个接口。**AI 提高的是效率，不应该降低系统的确定性。** 这条边界在真实联调时立刻见效：上游因账号配额返回 400 时，系统干净地降级为一句可读提示，手工出题完全不受影响。

**一个关于「完成」的认识：「测试通过」和「可以交付」之间的距离，只能靠把自己当用户走一遍来跨过。** 自动化测试断言的是接口契约，它不会告诉你管理员的首页该不该请求教师接口、一个被禁用的按钮在界面上看不看得出是禁用的、`RESULTS_PUBLISHED` 这个状态在首页应该显示成什么。所以本项目在 Chrome 上把管理员、教师、学生三条动线完整走了一遍，留存 39 张截图，让界面行为与接口响应互相印证。

**关于质量保证的做法。** 两人分头做实现与设计文档，一致性不能靠口头对齐，得让证据自己说话：把验收数字写死进测试断言、逐日记录改动与验证、浏览器走查留存截图、在真实数据库上把数字复现一遍。其中最有效的是第一条——20 分、28 分、`1、2、2、4` 这几个数字一旦写进断言，就不可能在不知情的情况下改坏判分逻辑。

**关于工程取舍的记录方式。** 3.6 节的 33 条决策每条都写成「问题 → 做法 → 为什么不用另一种」，其中相当一部分是取舍而不是唯一正解：令牌不维护吊销名单而靠每次请求回查用户状态、批量导入刻意不加整批事务、自动组卷刻意让每次抽题给出不同组合。把「为什么不用另一种」写下来，比只写「做了什么」更经得起追问，也让后来改动的人知道哪条边界是有意画的。

**一次范围扩张给设计的检验。** 项目后期追加了规则自动组卷、错题本与重练、试卷导出和「系统设置改为可编辑」四项功能。这是对前期设计的一次真实检验：自动组卷没有新增写库路径（保存仍走原来的组卷接口）、重练没有碰成绩表、导出复用了导入的列格式、判分复用了交卷的比较器——**新功能全部挂在既有约束上，而不是各自开一条新路**。唯一一次真的需要改主意的是系统设置：它原先被明确设计成只读，理由写在代码注释里（「可编辑表单会造成界面改完但进程没重启」）。要开放编辑，没有推翻那个理由，而是把它变成一条筛选标准——只有「每次用到都会重新读取」的参数才进白名单，启动时固化的调度间隔和 HTTP 超时一律不开放。**当新需求与旧决定冲突时，先弄清旧决定在防什么，往往能找到一个两者都满足的做法。**
